defmodule Drum.PipelineTest do
  ...
  describe "step/3" do
    ...
    test "multiple sequential steps" do
      {:ok, pid} =
        Drum.new()
        |> Drum.step("step1", "echo first")
        |> Drum.step("step2", "echo second")
        |> run_pipeline()
    ...
    test "capture stdout" do
      {:ok, pid} =
        Drum.new()
        |> Drum.step("step1", "echo hello_stdout")
        |> run_pipeline()
    ...
    test "capture stderr" do
      {:ok, pid} =
        Drum.new()
        |> Drum.step("step1", "echo hello_stderr >&2")
        |> run_pipeline()
    ...
    test "non-zero command stops further execution" do
      {:ok, pid} =
        Drum.new()
        |> Drum.step("step1", "exit 1")
        |> Drum.step("step2", "echo should_not_run")
        |> run_pipeline()
    ...
    test "non-existent command fails pipeline" do
      {:ok, pid} =
        Drum.new()
        |> Drum.step("step1", "/doesnt-exist")
        |> run_pipeline()

      assert {:error, _} = await_pipeline(pid)
    end

    test "function step runs command via Drum.cmd!" do
      {:ok, pid} =
        Drum.new()
        |> Drum.step("step1", fn _ctx, cmd_opts -> Drum.cmd!("echo from_fn", cmd_opts) end)
        |> run_pipeline()
    ...
    test "function step with multiple Drum.cmd! calls runs them sequentially" do
      {:ok, pid} =
        Drum.new()
        |> Drum.step("step1", fn _ctx, cmd_opts ->
          Drum.cmd!("echo cmd_one", cmd_opts)
          Drum.cmd!("echo cmd_two", cmd_opts)
        end)
        |> run_pipeline()
    ...
    test "function step that raises fails the pipeline" do
      {:ok, pid} =
        Drum.new()
        |> Drum.step("step1", fn _ctx, _cmd_opts -> raise "boom" end)
        |> run_pipeline()
