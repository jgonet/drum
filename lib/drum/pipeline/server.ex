defmodule Drum.Pipeline.Server do
  use GenServer, restart: :temporary
  alias Drum.{Group, Output, Registry, Step, Utils}
  ...
  defp start_item(%Step{} = step, rest, state) do
    ...
    {:ok, _} = DynamicSupervisor.start_child(worker_sup, {Drum.Step.Server, step_server_args})
  ...
  defp start_item(%Group{} = group, rest, state) do
    ...
    {:ok, _} = DynamicSupervisor.start_child(worker_sup, {Drum.Group.Server, group_server_args})
    {:noreply, %{state | items: rest}}
  end
  ...
  defp notify_owner(state, result) do
    send(state.owner, {:drum_pipeline_result, state.pipeline_id, result})
  end
end
