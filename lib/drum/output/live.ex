defmodule Drum.Output.Live do
  @moduledoc false
  @behaviour Drum.Output

  alias Drum.TmpDir
  alias Drum.Output.Utils
  alias Drum.Utils, as: DrumUtils
  ...
  def handle_event({:pipeline_started, pipeline_id, data}, state) do
    ...
    drum_meta = Map.get(meta, :drum, %{})
    card_key = Map.get(drum_meta, :logical_id, {:pipeline, pipeline_id})
    label = Map.get(drum_meta, :subscription_name)
    run_n = Map.get(drum_meta, :run_n)
  ...
  def handle_event({:command_started, pipeline_id, data}, state) do
    ...
    update_card_run_state(state, pipeline_id, DrumUtils.now_ms(), fn run_state ->
      buffer = %{
        cmd: cmd,
        exit_code: nil,
  ...
  @doc false
  def render_dashboard(state, now_ms \\ DrumUtils.now_ms()) do
  ...
  defp render_block(state) do
    render_dashboard(state, DrumUtils.now_ms())
  end
    ...
    cond do
      ...
      true ->
        delays =
          state.watchers
          |> Map.values()
          |> Enum.map(&next_relative_age_delay(&1.last_update_ms, DrumUtils.now_ms()))
          |> Kernel.++(
            state.cards
            |> Map.values()
            |> Enum.flat_map(fn card ->
              Enum.map(
                ...
                &next_relative_age_delay(&1.last_seen_ms, DrumUtils.now_ms())
              )
            end)
          )
          |> Enum.reject(&is_nil/1)
